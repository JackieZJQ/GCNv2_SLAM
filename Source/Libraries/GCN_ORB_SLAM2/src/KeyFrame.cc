...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB)
    : mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS),
      ...
      mThDepth(F.mThDepth), 
      N(F.N), NDict(F.NDict),
      mvKeys(F.mvKeys), mvKeysDict(F.mvKeysDict),
      mvKeysUn(F.mvKeysUn), mvKeysUnDict(F.mvKeysUnDict), 
      mvuRight(F.mvuRight), mvuRightDict(F.mvuRightDict), 
      mvDepth(F.mvDepth), mvDepthDict(F.mvDepthDict),
      mDescriptors(F.mDescriptors.clone()), mDescriptorsDict(F.mDescriptorsDict),
      mvpMapPoints(F.mvpMapPoints), mvpMapPointsDict(F.mvpMapPointsDict),
      mGridDict(F.mGridDict),
      mBowVec(F.mBowVec),
